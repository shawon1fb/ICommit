...
actor OllamaService: AIServiceProtocol {
  ...
  private struct Config {
    ...
    static func loadFromEnvironment() -> Config {
      ...
      let scheme = ProcessInfo.processInfo.environment["OLLAMA_SCHEME"] ?? defaultScheme
      let model = ProcessInfo.processInfo.environment["OLLAMA_MODEL"] ?? ""
      ...
      guard let baseURL = urlComponents.url else {
        // Fallback to default URL if components are invalid
        return Config(
          baseURL: URL(string: "\(defaultScheme)://\(defaultHost):\(defaultPort)/api")!,
          model: model
        )
      }
  ...
  func setModel(_ modelName: String) async throws {
    ...
    guard availableModels.contains(modelName) else {
      throw CLIError.aiGenerationFailed(
        "Model '\(modelName)' is not available. Available models: \(availableModels.joined(separator: ", "))"
      )
    }
    model = modelName
